...
#' directory is created to store all generated tables, plots, and model objects. This
#' practice is crucial for organizational hygiene and separates the new, more rigorous
#' analysis from any prior work, a key principle of reproducible science.

# Define root directory (assumes the script is run from the project's root directory)
root_dir <- getwd() 

# Define output directory, matching convention from previous scripts
output_dir <- file.path(root_dir, "phase_new_analysis")
...
#' The raw dataset, exported from Qualtrics as a CSV file, is loaded into an R data frame.
#' Based on your directory structure, the script now looks for 'raw copy.csv' in the main project folder.
#' An initial inspection is performed to confirm successful data ingestion.

# Define file path for the raw data, located in the root directory
raw_data_file <- file.path(root_dir, "raw copy.csv")

# Load the data, skipping the first two metadata rows typical of Qualtrics exports
# and handling potential character encoding issues.
df_raw <- read.csv(raw_data_file, header = TRUE, skip = 2, stringsAsFactors = FALSE)

# Initial inspection of the data structure
cat("Raw data loaded. Dimensions:", nrow(df_raw), "rows,", ncol(df_raw), "columns.\n")
# glimpse(df_raw) # Uncomment to see a detailed glimpse of the data
...
#' column from the raw data. We also select only the relevant columns for analysis.
...
#' This section transforms raw item responses into meaningful composite scores for each
#' psychological construct. The item-to-scale mappings are based on the preliminary
#' analysis script and established scoring protocols.

# Define subscale items based on preliminary analysis
...
#' validity analyses. This section provides a placeholder for reverse-scoring logic.
#' **Note:** Based on the SACS development papers, no items are reverse-scored.
...
#' are compared to the preliminary report.
...
#' The replicated CFA confirms the findings from the preliminary analysis.
...
#' clinical samples, necessitates an exploratory approach to understand the
...
#' This analysis represents a key novel contribution of the thesis.
...
#' and a significant Bartlett's test are required to proceed.
...
#' method. Because the original SACS factors are theoretically correlated,
#' an **oblique rotation** (`rotate = "oblimin"`) is used. This allows the factors to
#' correlate, providing a more realistic representation of psychological constructs.
...
#' more appropriate in some clinical samples.
...
#' Values of alpha >.70 are generally considered acceptable.
...
#' (Q-Q plots) and statistical (Shapiro-Wilk test) methods.
...
#' multicollinearity issues ahead of regression.
...
#' The preliminary analysis suggested that SACS_Transcending adds little predictive
#' value over SACS_Centering. Hierarchical regression is the ideal method to formally
#' test this "incremental validity". We build nested models
...
#' normality of residuals, and influential cases.
...
#' mental health. This section tests a plausible mediation model:
...
#' which is superior to older methods.
...
#' (listwise deletion) can reduce power and introduce bias. This section
...
#' MI is the gold standard for handling data that is Missing At Random (MAR).
